...
function doRemoveDatasetClients(datasetClientsToRemove, cb) {
  ...
  debug('doRemoveDatasetClients removeIds.length = %j', removeIds.length);
  async.eachSeries(removeIds, doRemoveDatasetClientWithRecords, function(err, deleteResult) {
    if (err) {
...
function upsertOrDeleteDatasetRecords(datasetId, datasetClientId, records, cb) {
  ...
  debug('upsertOrDeleteDatasetRecords diff records.length = %j', records.length);
